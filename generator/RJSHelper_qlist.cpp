  ...
  // ------------
      ...
      QJSValue RJSHelper::cpp2js_QList_QStringList(RJSApi& handler, const QList<QStringList>& v) {
          QJSEngine* engine = handler.getEngine();
          QJSValue ret = engine->newArray((uint)v.length());
          for (int i=0; i<v.length(); i++) {
              QJSValue jv = RJSHelper::cpp2js_QStringList(handler, v.at(i));
              // prevent undefined values from C++ (e.g. QObjects that are not included in result):
              if (!jv.isUndefined()) {
                  ret.setProperty((quint32)i, jv);
              }
      ...
      QList<QStringList> RJSHelper::js2cpp_QList_QStringList(RJSApi& handler, const QJSValue& v) {
          //return engine->fromScriptValue<QList<QStringList>>(v);
          QList<QStringList> ret;

          if (!v.isArray()) {
              qWarning() << "js2cpp_QList_QStringList: value is not an array";
              return ret;
          }

          const int length = v.property("length").toInt();
          for (int i=0; i<length; ++i) {
              ret.append(js2cpp_QStringList(handler, v.property(i)));
          }

          return ret;
      }

      bool RJSHelper::is_QList_QStringList(RJSApi& handler, const QJSValue& v, bool acceptUndefined) {
          if (v.isUndefined() || v.isNull()) {
              return acceptUndefined;
          }
          return v.isArray();
